...
export default class Horizon extends InsituVis {
  constructor(token) {
    ...
    var padding = 2
    this.state.horizon = d3.horizon()
      .width(this.state.width-padding)
      .height(this.state.height-padding)
      .bands(4)
      .mode('mirror')
      .interpolate('basis')
      .colors([
        this.state.color(0.05), 
        this.state.color(0.5), 
...
/********************************************************
 * NOTE: The following code is d3.horizon the d3 plugin *
 * I wasn't sure how to incorporate it properly in this *
 * code... need to figure that out or port to something *
 ********************************************************/
